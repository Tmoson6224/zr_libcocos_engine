...
static int calculate_lcm_and_needs_backwards_lcmed(size_t backwards_multiple, size_t received_size, size_t size_to_achieve,
...
static void *internal_grow_both_sides(mstate m, allocation_type command, void *oldmem, size_t minbytes, size_t maxbytes, size_t *received_size, size_t backwards_multiple, int only_preferred_backwards)
{
      ...
      */
      ...
      }
      {
         ...
         else
         {
            ...
            and the alignment*/
            if (!calculate_lcm_and_needs_backwards_lcmed(backwards_multiple, *received_size, only_preferred_backwards ? maxbytes : minbytes, &lcm, &needs_backwards_lcmed) || !RTCHECK(ok_address(m, prev)))
            {
               USAGE_ERROR_ACTION(m, oldp);
               return 0;
            }
   ...
   *received_size = chunksize(mem2chunk(oldmem)) - overhead_for(mem2chunk(oldmem));
...

   {
                  ...
                  ok_next(oldp, next) && ok_pinuse(next)))
      {
         ...
         { // nbmin <= oldsize /* already big enough*/
            size_t nb = nbmin;
            size_t rsize = oldsize - nb;
            if (rsize >= MIN_CHUNK_SIZE)
            {
...
static int internal_node_multialloc(mstate m, size_t n_elements, size_t element_size, size_t contiguous_elements, boost_cont_memchain *pchain)
{
...
static void internal_multialloc_free(mstate m, boost_cont_memchain *pchain)
{
   ...
   if (!PREACTION(m))
   {
      ...
      while (!BOOST_CONTAINER_MEMCHAIN_IS_END_IT(pchain, a_it))
      { /* Iterate though all memory holded by the chain */
         ...
         {
            while (1)
            { /* Internal loop to speed up forward and backward merging (avoids some redundant checks) */
...
#if FOOTERS
   {
      ...
      BOOST_CONTAINER_MEMCHAIN_INCORPORATE_AFTER(pchain, last_pchain, BOOST_CONTAINER_MEMCHAIN_FIRSTMEM(&ret_chain), BOOST_CONTAINER_MEMCHAIN_LASTMEM(&ret_chain), BOOST_CONTAINER_MEMCHAIN_SIZE(&ret_chain));
   }
#endif
}
...
static int internal_multialloc_arrays(mstate m, size_t n_elements, const size_t *sizes, size_t element_size, size_t contiguous_elements, boost_cont_memchain *pchain)
{
   void *mem;             /* malloced aggregate space */
   mchunkptr p;           /* corresponding chunk */
   size_t remainder_size; /* remaining bytes while splitting */
...
int boost_cont_multialloc_arrays(size_t n_elements, const size_t *sizes, size_t element_size, size_t contiguous_elements, boost_cont_memchain *pchain)
{
   int ret = 0;
   mstate ms = (mstate)gm;
...
void *boost_cont_malloc(size_t bytes)
{
   ...
   ensure_initialization();
   return boost_cont_allocation_command(BOOST_CONTAINER_ALLOCATE_NEW, 1, bytes, bytes, &received_bytes, 0).first;
}
...
int boost_cont_multialloc_nodes(size_t n_elements, size_t elem_size, size_t contiguous_elements, boost_cont_memchain *pchain)
{
   int ret = 0;
   mstate ms = (mstate)gm;
   ensure_initialization();
...
int boost_cont_grow(void *oldmem, size_t minbytes, size_t maxbytes, size_t *received)
{
   mstate ms = (mstate)gm;
   if (!ok_magic(ms))
   {
...
int boost_cont_shrink(void *oldmem, size_t minbytes, size_t maxbytes, size_t *received, int do_commit)
{
   mstate ms = (mstate)gm;
   if (!ok_magic(ms))
   {
...
void *boost_cont_alloc(size_t minbytes, size_t preferred_bytes, size_t *received_bytes)
{
   // ensure_initialization provided by boost_cont_allocation_command
   return boost_cont_allocation_command(BOOST_CONTAINER_ALLOCATE_NEW, 1, minbytes, preferred_bytes, received_bytes, 0).first;
}
...
boost_cont_command_ret_t boost_cont_allocation_command(allocation_type command, size_t sizeof_object, size_t limit_size, size_t preferred_size, size_t *received_size, void *reuse_ptr)
{
   boost_cont_command_ret_t ret = {0, 0};
   ensure_initialization();
   if (command & (BOOST_CONTAINER_SHRINK_IN_PLACE | BOOST_CONTAINER_TRY_SHRINK_IN_PLACE))
   {
      int success = boost_cont_shrink(reuse_ptr, preferred_size, limit_size, received_size, (command & BOOST_CONTAINER_SHRINK_IN_PLACE));
   ...
   *received_size = 0;
...

   {
      ...
      if (!PREACTION(ms))
      {
         ...
         {
            void *r = internal_grow_both_sides(ms, command, reuse_ptr, limit_size, preferred_size, received_size, sizeof_object, 1);
            if (r)
            {
               ret.first = r;
               ret.second = 1;
         ...
         if (reuse_ptr && (command & (BOOST_CONTAINER_EXPAND_FWD | BOOST_CONTAINER_EXPAND_BWD)))
         {
            void *r = internal_grow_both_sides(ms, command, reuse_ptr, limit_size, preferred_size, received_size, sizeof_object, 0);
            if (r)
            {
               ret.first = r;
               ret.second = 1;
